...
Double_t dirFit::calDirLikelihood( RooListProxy* _pulls, std::vector<wbEvent::wbHit> hitlist, std::vector<double> vertex, vector<wbPDF*> pdf, bool twodpdf, bool doCharge, bool doCos, int ntimebin ) const{
  ...
  theta = ((RooAbsReal*)_pulls->at(0))->getVal();
  phi   = ((RooAbsReal*)_pulls->at(1))->getVal();
  //cout<<"Dir likelihood theta phi "<<theta<<" "<<phi<<" hitlist size "<<hitlist.size()<<endl;
  ...
  for (wbEvent::wbHit i: hitlist){
    ...
    if (h1->Integral() == 0 || !h1->Integral()) return 1e9;
    //if (h1->Integral() == 0 || !h1->Integral() ){ cout<<"2D per pmt pdf has not been set ! Exit ! "<<endl; exit(0);}
    //cout<<"h1 integrated value "<<h1->Integral()<<endl;
    h1->Scale(200./h1->Integral());
    //TH1D* hh = h1->ProjectionX();
    //for (int ibin = 0; ibin<46;ibin++){
      //cout<<"bin "<<ibin<<" content "<<hh->GetBinContent(ibin+1)<<endl;
    //}
/*    
    TGraph2D pdf_pmt(h1);
...
*/
    TGraph pdf_pmt(h1->ProjectionX());

    double user_weight = 0.1;
    if (pmtid>=231) user_weight = 0;

    double currll;
    double temp;
    if (_ifScan){
      if (doCharge) { temp = pdf_pmt.Eval(pmtid) * i.charge * user_weight ;}
      else {temp = pdf_pmt.Eval(pmtid) *user_weight ;}
    }
    else {
      if (doCharge) { temp = pdf_pmt.Eval(pmtid) * i.charge * user_weight ;}
      else {temp = pdf_pmt.Eval(pmtid) * user_weight  ;}
